...
**** TODO See if I can create my own custom action function that makes it easy to standardise buffer handling

e.g. a ~me/display-in-bottom-side-window~ action that

- [ ] Will display the buffer in a bottom side window
- [ ] Will enable ~tab-line-mode~ for that buffer
- [ ] Will set  ~no-other-window . t~ in the window's parameters so that it
  cannot be selected through =C-x o=
